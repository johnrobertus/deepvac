...
const ThermalVision = () => {
  ...
  const features = t("thermalVision.features", {
    returnObjects: true,
  }) as Array<{ label: string; detail: string }>;

  const specs = t("thermalVision.specs", {
    returnObjects: true,
      ...
      <PageShell>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 pt-4">
            <TechChip
              label={t("thermalVision.chips.environment.label")}
              value={t("thermalVision.chips.environment.value")}
            />
            <TechChip
              label={t("thermalVision.chips.enclosure.label")}
              value={t("thermalVision.chips.enclosure.value")}
            />
            <TechChip
              label={t("thermalVision.chips.compatibility.label")}
              value={t("thermalVision.chips.compatibility.value")}
            />
          </div>
        </PageHero>
            ...
            <div className="space-y-6">
              <div className="relative rounded-lg overflow-hidden border border-gray/10">
                ...
                <div className="absolute top-3 left-3 glass-overlay rounded-md px-3 py-1.5">
                  <span className="mono-label text-blue-light">
                    {t("thermalVision.imageLabel")}
                  </span>
                </div>
              </div>
