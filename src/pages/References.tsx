...
const References = () => {
  ...
  const caseStudies = t("caseStudies.items", { returnObjects: true }) as Array<{ category: string; title: string; summary: string }>;
  ...
  return (
    <Layout>
      ...
      <PageShell>
        ...
        <Section>
          <SectionHeader eyebrow={t("industries.eyebrow")} title={t("industries.title")} description={t("industries.description")} className="mb-14" />
        ...
        <Section className="bg-surface/30">
          <SectionHeader eyebrow={t("projectScope.eyebrow")} title={t("projectScope.title")} description={t("projectScope.description")} className="mb-10" />
        ...
        <Section>
          <SectionHeader eyebrow={t("collaborations.eyebrow")} title={t("collaborations.title")} description={t("collaborations.description")} className="mb-10" />
        ...
        <Section className="bg-surface/30">
          <SectionHeader eyebrow={t("caseStudies.eyebrow")} title={t("caseStudies.title")} description={t("caseStudies.description")} className="mb-10" />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {caseStudies.map((study, i) => (
              <div key={i} className="bento-card rounded-lg overflow-hidden">
                ...
                <div className="p-6 space-y-3">
                  ...
                  <span className="flex items-center gap-1.5 text-xs text-gray/40 font-mono">{t("caseStudies.comingSoon")} <ArrowRight className="w-3 h-3" /></span>
                </div>
              </div>
            ))}
          </div>
        ...
        <CTABand title={t("cta.title")} description={t("cta.description")}>
          <Button asChild><Link to={localizedPath("/contact", lang)}>{tc("buttons.requestConsultation")}</Link></Button>
          <Button asChild variant="outline"><Link to={localizedPath("/products", lang)}>{tc("buttons.exploreProducts")}</Link></Button>
        </CTABand>
      </PageShell>
    </Layout>
  );
