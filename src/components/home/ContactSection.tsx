...
export function ContactSection() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.firstName.trim()) errors.firstName = t("form.validation.firstNameRequired");
    if (!form.lastName.trim()) errors.lastName = t("form.validation.lastNameRequired");
    if (!form.email.trim()) {
      errors.email = t("form.validation.emailRequired");
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(form.email.trim())) {
      errors.email = t("form.validation.emailInvalid");
    }
    if (!form.company.trim()) errors.company = t("form.validation.companyRequired");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!consent) { toast.error(t("form.validation.consentRequired")); return; }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("send-inquiry", {
        body: { firstName: form.firstName, lastName: form.lastName, email: form.email, phone: form.phone || undefined, company: form.company, project: form.project || undefined, message: form.message || undefined, source: "homepage-contact", _website: honeypot, turnstileToken: turnstileToken || undefined },
      });
      if (error) throw error;
      if (data?.error) {
        const msg = data.error as string;
        if (msg.includes("Message must be") || msg.includes("Missing required") || msg.includes("Invalid email")) { toast.error(msg); } else { throw new Error(msg); }
        return;
      }
      setSubmitted(true);
      setValidationErrors({});
      ...
      if (err?.message?.includes("Too many requests")) { toast.error(t("form.errors.tooManyRequests")); } else { toast.error(t("form.errors.submissionFailed")); }
      ...
      <section className="py-20 md:py-28 px-6 bg-surface/30">
        <div className="container max-w-6xl">
          <div className="max-w-xl mx-auto text-center space-y-6">
            ...
            <p className="text-gray text-sm leading-relaxed">{t("form.success.message")}</p>
            <Button variant="outline" onClick={() => { setSubmitted(false); setForm(initialForm); setConsent(false); }}>
              {t("buttons.submitAnotherInquiry")}
            </Button>
          </div>
        </div>
      </section>
          ...
          <Reveal delay={100}>
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <FormField label={t("form.firstName")} placeholder={t("form.placeholders.firstName")} required value={form.firstName} onChange={set("firstName")} error={validationErrors.firstName} />
                <FormField label={t("form.lastName")} placeholder={t("form.placeholders.lastName")} required value={form.lastName} onChange={set("lastName")} error={validationErrors.lastName} />
              </div>
              <FormField label={t("form.workEmail")} placeholder={t("form.placeholders.email")} type="email" required value={form.email} onChange={set("email")} error={validationErrors.email} />
              <FormField label={t("form.company")} placeholder={t("form.placeholders.company")} required value={form.company} onChange={set("company")} error={validationErrors.company} />
              ...
              <div aria-hidden="true" style={{ position: "absolute", left: "-9999px", top: "-9999px", opacity: 0, height: 0, overflow: "hidden" }}>
                ...
                <input type="text" id="hp-website" name="website" tabIndex={-1} autoComplete="off" value={honeypot} onChange={(e) => setHoneypot(e.target.value)} />
              </div>
          ...
          <Reveal delay={200}>
            <div className="space-y-5 lg:pl-8">
              ...
              <a href="https://www.linkedin.com/company/deepvac-gmbh/" target="_blank" rel="noopener noreferrer"
                ...
                <span className="text-sm text-gray group-hover:text-sand transition-colors">{t("footer.followDeepvac", { ns: "common" })}</span>
